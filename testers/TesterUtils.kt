...
internal class Result {
    ...
    @Throws
    constructor(testNo: Int, input: String, output: String?, expected: String?) {
        // This throw is debatable but I wanted to have it here for demonstration purposes
        if (testNo < 1)
                throw IllegalArgumentException(
                        "A test number of $testNo is invalid. The test number should be >= 1."
                )
        this.testNo = testNo
        this.input = input
        this.output = output
    ...
    fun print() {
        val resultString =
                if (passed) "Test $testNo: PASSED - Input: $input"
                else
...
internal fun <T : Comparable<T>> buildResult(
        testNo: Int,
        input: String,
        output: T,
        expected: T
...
private fun <T : Comparable<T>> verify(
        output: List<T>,
        expected: List<T>,
        ordered: Boolean
): Boolean {
